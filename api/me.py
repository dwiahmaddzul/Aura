...
liked posts, bookmarks, daily check-in prompt, onboarding state.
"""
import random
import time
...
from utils import time_ago, part_of_day_now, greeting_for_now
...
def api_me_profile():
    ...
    # Detect onboarding state — first-time user has 0 posts
    posts_count = db.execute(
        "SELECT COUNT(*) AS n FROM posts WHERE username='me'"
    ).fetchone()["n"]
    if not row:
        return jsonify(
            {
