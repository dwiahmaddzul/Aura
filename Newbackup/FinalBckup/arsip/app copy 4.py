...
def schedule_responses(pid, content, img=None, poster="me"):
    """Schedule AI persona responses. poster = who made the post (skip self-reply)"""
    is_img = bool(img)
    for p in PERSONAS:
...
def ai_post_scheduler():
    """Background: AI personas post on timeline periodically"""
    time.sleep(120)  # wait 2min after startup
    while True:
        try:
            p = random.choice(PERSONAS)
            topic = random.choice(p["post_topics"])
            print(f"[AI-Post] {p['username']} generating post about '{topic}'...")
            content = call_llm([
...
async function openProfile(username){
  try{
    ...
    document.getElementById('profContent').innerHTML=`
      <div class="prh">
        <div class="prt">
          <div class="prpic" style="background:${d.color}">${d.avatar}</div>
          <div class="prs">
