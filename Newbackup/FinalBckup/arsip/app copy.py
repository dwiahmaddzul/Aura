...
# Model untuk teks biasa
TEXT_MODEL = "Qwen/Qwen2.5-72B-Instruct"
# Model untuk VLM (vision)
...
PERSONAS = [
    {
        ...
        "personality": (
            "Kamu adalah Maya, seorang seniman digital dan desainer grafis berusia 24 tahun dari Bandung. "
            "Suka hal-hal estetis, sering pakai kata-kata kayak 'aesthetic banget', 'literally', 'no cap'. "
            "Responsmu singkat, natural, kadang pakai emoji. Bahasa Indonesia gaul anak muda."
        ),
        "delay_range": (30, 120),   # detik
    },
    {
        "username": "rizky_dev",
        "display":  "Rizky 💻",
        ...
        "personality": (
            "Kamu adalah Rizky, software developer berusia 27 tahun dari Jakarta. "
            "Tech-savvy, suka bikin meme, sering ada referensi coding/teknologi tapi tetap relate. "
            "Responsnya witty dan sedikit nerd tapi friendly. Suka pakai 'anjir', 'gila', 'wkwkwk'. "
            "Bahasa Indonesia campur sedikit Inggris (Jaksel-style)."
        ),
        "delay_range": (60, 180),
    },
    {
        ...
        "personality": (
            "Kamu adalah Nadia, food blogger dan content creator berusia 22 tahun dari Surabaya. "
            "Super positif, supportif, suka kasih semangat. Banyak pakai 'Yaampun!', 'Ih pengen!', 'Keren bangettt'. "
            "Kalau lihat foto makanan selalu excited. Bahasa Indonesia expressive dengan banyak tanda seru."
        ),
        "delay_range": (20, 90),
    },
    {
        ...
        "personality": (
            "Kamu adalah Bimo, gamer dan streamer berusia 25 tahun dari Yogyakarta. "
            "Santai, suka bercanda, sering bikin jokes ringan. Pakai 'gg', 'ez', 'bruh', 'bro'. "
            "Terkadang relate semua hal ke game. Bahasa Indonesia casual, sering nge-roast tapi dengan kasih sayang."
        ),
        "delay_range": (90, 240),
    },
]
...
def call_llm(messages: list, model: str = TEXT_MODEL, max_tokens: int = 200) -> str:
    """Panggil SiliconFlow API (OpenAI-compatible)."""
    headers = {
            ...
            "content": [
                ...
                },
                {
                    ...
                    "text": (
                        ...
                        "Lihat gambar ini yang baru diposting teman di social media. "
                        "Tulis komentar singkat dan natural (1-2 kalimat) sesuai karaktermu. "
                        "Jangan terlalu formal. Berikan reaksi yang genuine."
                    ),
                },
            ],
        }
...
def generate_text_comment(post_content: str, persona: dict) -> str:
    ...
    messages = [
        ...
        },
        {
            "role": "user",
            "content": (
                f"Teman kamu baru posting di social media:\n\n"
                f"\"{post_content}\"\n\n"
                "Tulis komentar singkat dan natural (1-2 kalimat) sesuai karaktermu. "
                "Jangan mulai dengan 'Komentar:' atau sejenisnya. Langsung komentarnya saja."
            ),
        },
    ]
    return call_llm(messages, model=TEXT_MODEL, max_tokens=150)


# ─── BACKGROUND AI SCHEDULER ─────────────────────────────────────────────────
def schedule_ai_responses(post_id: int, post_content: str, image_b64: str = None):
    """
    Pilih beberapa AI persona secara acak dan jadwalkan
    komentar mereka dengan delay random (natural timing).
    """
    # Pilih 2-3 persona secara acak
    selected = random.sample(PERSONAS, k=random.randint(2, 3))
...
def create_post():
    ...
    image_b64 = data.get("image_b64")      # base64 string dari frontend
    ...
    # Kalau ada gambar, minta VLM buat deskripsi dulu (untuk context)
    image_desc = None
    if image_b64:
        print("[VLM] Menganalisis gambar...")
        desc_messages = [
            {
                "role": "user",
                "content": [
                    ...
                    {
                        "type": "text",
                        "text": "Deskripsikan gambar ini dalam 1-2 kalimat singkat dalam bahasa Indonesia.",
                    },
                ],
            }
        ]
        image_desc = call_llm(desc_messages, model=VISION_MODEL, max_tokens=100)
        print(f"[VLM] Deskripsi: {image_desc}")

    post_id = db.execute(
        "INSERT INTO posts (username, content, image_b64, image_desc, post_type, created_at) VALUES (?,?,?,?,?,?)",
        ("me", content, image_b64, image_desc, post_type, time.time()),
    ).lastrowid
    db.commit()

    # Jadwalkan AI responses di background
    context = content or image_desc or "foto baru"
    schedule_ai_responses(post_id, context, image_b64)
...
async function submitPost() {
  ...
  btn.disabled = true;
  btn.textContent = currentImageB64 ? '🤖 Analyzing...' : 'Posting...';
...
if __name__ == "__main__":
    ...
    print(f"  API Key     : {'✅ Set' if not SILICONFLOW_API_KEY.startswith('sk-GANTI') else '❌ BELUM DISET!'}")
    print("=" * 55)
